...
class Settings:
    ...
    tool_result_limit: int  # 单个工具结果字符上限（供应商候选 JSON 较大）
    ...
    # ---- 四期：存储 ----
    # 默认 SQLite（零外部依赖，落在 DATA_DIR/sourcepilot.db）。
    ...
    # ---- B2B sourcing ----
    qdrant_supplier_collection: str = "sourcepilot_suppliers"
...
def load_settings() -> Settings:
    ...
    return Settings(
        ...
        qdrant_collection=os.getenv("QDRANT_COLLECTION", "sourcepilot_products"),
        reranker_base_url=os.getenv("RERANKER_BASE_URL", ""),
        reranker_model=os.getenv("RERANKER_MODEL", ""),
        tavily_api_key=os.getenv("TAVILY_API_KEY", ""),
        otlp_endpoint=os.getenv("OTEL_EXPORTER_OTLP_ENDPOINT", ""),
        data_dir=data_dir,
        category_kb_collection=os.getenv("CATEGORY_KB_COLLECTION", "sourcepilot_category_kb"),
        ...
        database_url=(
            ...
            or f"sqlite+aiosqlite:///{data_dir / 'sourcepilot.db'}"
        ),
        ...
        qdrant_supplier_collection=os.getenv("QDRANT_SUPPLIER_COLLECTION", "sourcepilot_suppliers"),
    )
