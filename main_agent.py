# -*- coding: utf-8 -*-
"""Procurement Supervisor（SourcePilot primary surface）

SourcePilot 总调度。基于 AgentScope 2.0 Agent，B2B 主流程工具集分层：
    1. supplier_search_tool 直接处理简单单品类寻源；
...
每个 procurement_session_id 对应一个 MainAgent 实例，由 SessionRegistry 缓存；
...
class MainAgentFactory:
    ...
    def build(self, restored_state: Optional[AgentState] = None) -> Agent:
        ...
        # SourcePilot 的公开产品面只注册 sourcing 工具；其他内部工具不进入主链路。
        business_tools = list(self._sourcing_factory.build_tools())
            ...
            *business_tools,
            ...
            FunctionTool(
                build_remember_preference_tool(
                    ...
                    self._bus,
                    statement_validator=validate_procurement_preference_statement,
                ),
                middlewares=self._resilience(),
            ),
...
class SessionRegistry:
    """按 procurement_session_id 缓存 MainAgent 实例，支撑多轮对话；
    ...
    async def get_or_create(self, procurement_session_id: str) -> Agent:
        if procurement_session_id not in self._agents:
            restored_state = await self._try_restore(procurement_session_id)
            self._agents[procurement_session_id] = self._main_factory.build(restored_state)
        return self._agents[procurement_session_id]

    async def persist(self, procurement_session_id: str) -> None:
        """每轮对话结束后落盘 AgentState 快照；失败仅告警不影响主链路。"""
        agent = self._agents.get(procurement_session_id)
        if agent is None:
            return
        try:
            await self._session_store.save(procurement_session_id, agent.state.model_dump_json())
        except Exception as err:  # noqa: BLE001
            logger.warning("会话状态落盘失败：%s（%s）", procurement_session_id, err)

    async def _try_restore(self, procurement_session_id: str) -> Optional[AgentState]:
        try:
            raw = await self._session_store.load(procurement_session_id)
        except Exception as err:  # noqa: BLE001 —— 存储不可用时按新会话继续，不阻断对话
            logger.warning("会话状态读取失败，按新会话处理：%s（%s）", procurement_session_id, err)
            return None
        if raw is None:
            return None
        try:
            state = AgentState.model_validate_json(raw)
            logger.info("会话状态已恢复：%s（%d 条上下文）", procurement_session_id, len(state.context))
            return state
        except Exception as err:  # noqa: BLE001 —— 快照损坏按新会话处理
            logger.warning("会话状态恢复失败，按新会话处理：%s（%s）", procurement_session_id, err)
            return None
