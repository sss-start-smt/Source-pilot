...
买家身份从 ProcurementContext 取真实值，不信任模型入参。
...
def build_remember_preference_tool(
    ...
    bus: TradeEventBus,
    statement_validator: Optional[Callable[[str], str]] = None,
):
    async def remember_preference_tool(
        ...
        try:
            normalized_statement = statement_validator(statement) if statement_validator else statement.strip()
            await store.append(BuyerPreference(buyer_id=buyer_id, kind=kind, statement=normalized_statement))
        ...
        bus.publish(
            session_id,
            "tool.result",
            {"tool": "remember_preference_tool", "saved": normalized_statement},
        )
        return ToolChunk(
            content=[TextBlock(type="text", text=f"已记住长期偏好：[{kind}] {normalized_statement}")],
