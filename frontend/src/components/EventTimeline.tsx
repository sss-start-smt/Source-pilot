...
const LABELS: Record<string, string> = {
  ...
  "tool.result": "工具完成",
  "plan.update": "任务清单",
  "workflow.progress": "采购进度",
  "memory.applied": "已应用长期偏好",
...
function summarize(event: TradeEvent): string {
  ...
  switch (event.type) {
    ...
    case "workflow.progress":
      return String(p.message ?? p.stage ?? "");
    case "memory.applied":
      return (p.preferences ?? []).join(" · ");
...
export default function EventTimeline({ events }: { events: TradeEvent[] }) {
  return (
    <aside className="timeline">
      <h2>事件时间线</h2>
      {events.length === 0 && <p className="empty">提交采购需求后，这里会实时显示 RFQ、寻源、过滤、报价与 Shortlist 进度。</p>}
      <ol>
