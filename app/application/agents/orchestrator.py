...
应用层编排入口：
    1. 把会话快照写入 ProcurementContext（ContextVar，工具与子 Agent 透明可读）；
...
class MainAgentOrchestrator:
    ...
    async def handle_intent(self, intent: SubmitIntentInput) -> SubmitIntentOutput:
        session_id = intent.procurement_session_id
        ...
        try:
            ...
            if cached is not None:
                ...
                return SubmitIntentOutput(procurement_session_id=session_id, final_text=final_text)
            ...
            # 阶段相关的顺序记录由 SequencingTracker 按会话保留，
            # 便于跨轮判断报价归一与比较的前置关系。
