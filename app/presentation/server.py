# -*- coding: utf-8 -*-
"""SourcePilot FastAPI 服务入口

主要路由：
    POST /procurement/intents              提交采购意图（同步返回最终回复）
    POST /procurement/intents/async        提交采购意图（立即返回 task_id）
    GET  /procurement/tasks/{task_id}      查任务状态（queued / running / done / failed）
    WS   /procurement/events               订阅会话事件流
...
同步接口用于本地演示和离线回归；长任务可使用异步接口与事件流。
"""
...
from app.presentation.dto import SubmitIntentRequest, SubmitIntentResponse
...
def build_app() -> FastAPI:
    ...
    api = FastAPI(title="SourcePilot Procurement Copilot", version="1.0.0-mvp", lifespan=lifespan)
    ...
    @api.post("/procurement/intents", response_model=SubmitIntentResponse)
    async def submit_intent(body: SubmitIntentRequest) -> SubmitIntentResponse:
        c = container()
        session_id = body.procurement_session_id or f"session-{uuid.uuid4().hex[:8]}"
        ...
        if c.task_queue is None:
            ...
            return SubmitIntentResponse(
                procurement_session_id=result.procurement_session_id, final_text=result.final_text,
            )

        task_id = await _enqueue(c, intent)
        final_text = await _await_result(c, task_id, session_id)
        return SubmitIntentResponse(procurement_session_id=session_id, final_text=final_text)

    @api.post("/procurement/intents/async")
    async def submit_intent_async(body: SubmitIntentRequest) -> dict:
        ...
        if c.task_queue is None:
            raise HTTPException(status_code=503, detail="队列未启用，请使用 /procurement/intents")
        task_id = await _enqueue(c, intent)
        return {"procurement_session_id": session_id, "task_id": task_id, "state": "queued"}

    @api.get("/procurement/tasks/{task_id}")
    ...
    @api.websocket("/procurement/events")
    async def procurement_events(websocket: WebSocket) -> None:
...
async def _queue_priority(c: Container, session_id: str) -> int:
    ...
    if not c.settings.queue_priority_enabled or not c.cache.enabled:
        return 0
    key = f"sourcepilot:turns:{session_id}"
    try:
...
async def _enqueue(c: Container, intent: SubmitIntentInput) -> str:
    ...
    这样可以避免重复模型调用、重复事件和重复评测成本。
    """
    fingerprint = hashlib.sha256(
        f"{intent.procurement_session_id}\n{intent.raw_query}".encode(),
    ).hexdigest()[:32]
    ...
    await c.task_queue.enqueue(  # type: ignore[union-attr]
        IntentTask(
            task_id=task_id,
            procurement_session_id=intent.procurement_session_id,
            buyer_id=intent.buyer_id,
            locale=intent.locale,
            currency=intent.currency,
            raw_query=intent.raw_query,
            priority=await _queue_priority(c, intent.procurement_session_id),
        ),
    )
    await c.task_queue.set_status(TaskStatus(task_id=task_id, state="queued"))  # type: ignore[union-attr]
    c.bus.publish(intent.procurement_session_id, "task.queued", {"task_id": task_id})
    return task_id
