...
Presentation 层按 procurement_session_id 订阅后推送给前端 WebSocket。
...
事件类型与参考实现语义一一对应：
    ...
    workflow.progress   B2B RFQ/寻源/报价/Shortlist 产品进度
    memory.applied      本轮实际注入的长期采购偏好
...
class TradeEvent:
    ...
    def to_dict(self) -> dict:
        return {
            "procurement_session_id": self.procurement_session_id,
            "type": self.type,
    ...
    def from_dict(raw: dict) -> "TradeEvent":
        return TradeEvent(
            procurement_session_id=raw["procurement_session_id"],
            type=raw["type"],
...
class TradeEventBus:
    ...
    def subscribe(self, procurement_session_id: str) -> asyncio.Queue:
        queue: asyncio.Queue = asyncio.Queue()
        self._subscribers.setdefault(procurement_session_id, []).append(queue)
        return queue

    def unsubscribe(self, procurement_session_id: str, queue: asyncio.Queue) -> None:
        queues = self._subscribers.get(procurement_session_id, [])
        if queue in queues:
            queues.remove(queue)
        if not queues:
            self._subscribers.pop(procurement_session_id, None)

    def deliver_local(self, event: TradeEvent) -> None:
        """只投递给本进程订阅者（背板收到远端事件后走这里，避免回环广播）。"""
        for queue in self._subscribers.get(event.procurement_session_id, []):
            queue.put_nowait(event)

    def publish(self, procurement_session_id: str, event_type: TradeEventType, payload: Any) -> None:
        if event_type not in EVENT_TYPES:
            raise ValueError(f"未知事件类型：{event_type}")
        event = TradeEvent(
            procurement_session_id=procurement_session_id,
            type=event_type,
            payload=payload,
