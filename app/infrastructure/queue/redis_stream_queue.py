...
_STREAM = "sourcepilot:intents"
# 大请求（长会话）单独一条流：与正常流同一消费者组名，
# 但 xreadgroup 里正常流排在前面——Redis 按传入顺序返回，天然形成优先级。
_LARGE_STREAM = "sourcepilot:intents:large"
_DEAD_STREAM = "sourcepilot:intents:dead"
_GROUP = "sourcepilot-workers"
_STATUS_PREFIX = "sourcepilot:task:"
_STATUS_TTL = 3600
_EVENT_CHANNEL_PREFIX = "sourcepilot:events:"
...
class RedisEventBackplane:
    ...
    async def publish(self, event: TradeEvent) -> None:
        channel = f"{_EVENT_CHANNEL_PREFIX}{event.procurement_session_id}"
