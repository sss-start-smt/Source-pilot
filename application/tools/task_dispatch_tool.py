...
def build_task_dispatch_tool(
    ...
    async def _preference_hint(subagent_type: str, demands: str):
        ...
        B2B 的 sourcing_agent / quote_agent 以及旧 search_agent 可读取长期偏好；
        trade_agent 只按给定 product_id/sku_id 执行，不注入采购偏好。
        """
        if not (
            subagent_inject
            and preference_store
            and subagent_type in {"sourcing_agent", "quote_agent", "search_agent"}
        ):
            return None
    ...
    async def task_dispatch(
        subagent_type: Literal["sourcing_agent", "quote_agent", "search_agent", "trade_agent"],
        demands: str,
    ) -> ToolChunk:
        ...
        Args:
            subagent_type (`str`):
                子代理类型："sourcing_agent"（B2B 供应商寻源专家）、
                "quote_agent"（报价解析/比较专家）；"search_agent" / "trade_agent"
                仅保留旧 C 端兼容，不属于 B2B 主流程。
            demands (`str`):
        ...
        elif subagent_type == "search_agent":
